...
// ==================== بناء الفلتر بناءً على Query وفلتر المنطقة ====================
const buildActivityFilter = (query, regionFilter = {}) => {
  const filter = { $and: [] };
  ...
  if (query.status && query.status !== "الكل" && query.status !== "") {
    filter.$and.push({ status: query.status });
  }

  if (
    query.fundingSource &&
    query.fundingSource !== "الكل" &&
    query.fundingSource !== ""
  ) {
    filter.$and.push({ fundingSource: query.fundingSource });
  }
...
const AddNewActivity = async (req, res) => {
  try {
    ...
    const newActivityData = {
      ...otherFields,
      activityCode: req.body.activityCode.toUpperCase(),
      governorate: req.body.governorate.trim(), // للتأكد من استخدام الحقل الصحيح في الـ schema
      contractualDocuments: [],
      activitypdfs: [],
      images: [],
      roaddetails: processedRoadDetails,
...
const getTotalDisbursed = async (req, res) => {
  try {
    ...
    let totalDisbursed = activities.reduce((total, activity) => {
      let extractsToSum = activity.extract;

      if (targetFiscalYear) {
        extractsToSum = activity.extract.filter(
          (ex) => ex.extractFiscalYear === targetFiscalYear
        );
      }

      const activityTotal = extractsToSum.reduce(
